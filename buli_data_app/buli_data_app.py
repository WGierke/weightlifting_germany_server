...
def add_schedule_to_cache(relay, schedule_dict):
    if not memcache.add('{}:schedule'.format(relay), json.dumps(schedule_dict, default=json_serial),
                        time=60 * 60 * 24 * 7):
        logging.error('Memcache set failed for schedule ' + relay)


def add_competition_to_cache(relay, competition_dict):
    if not memcache.add('{}:competitions'.format(relay), json.dumps(competition_dict, default=json_serial),
                        time=60 * 60 * 24 * 7):
